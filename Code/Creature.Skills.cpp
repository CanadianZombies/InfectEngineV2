...
DefineCommand ( cmd_gain )
{
	...
	if ( arg[0] == '\0' ) {
		cmd_function ( trainer, &cmd_say, "Pardon me?" );
		return;
	}
	...
	if ( !str_prefix ( arg, "list" ) ) {
		...
		for ( gn = 0; gn < MAX_GROUP; gn++ ) {
			...
			if ( !ch->pcdata->group_known[gn]
				...
				snprintf ( buf, sizeof ( buf ), "%-18s %-5d ",
						   group_table[gn].name, group_table[gn].rating[ch->archetype] );
				writeBuffer ( buf, ch );
				if ( ++col % 3 == 0 )
				{ writeBuffer ( "\n\r", ch ); }
			}
		...
		for ( sn = 0; sn < MAX_SKILL; sn++ ) {
			...
			if ( !ch->pcdata->learned[sn]
				...
				snprintf ( buf, sizeof ( buf ), "%-18s %-5d ",
						   skill_table[sn].name, skill_table[sn].rating[ch->archetype] );
				writeBuffer ( buf, ch );
				if ( ++col % 3 == 0 )
				{ writeBuffer ( "\n\r", ch ); }
			}
	...
	if ( gn > 0 ) {
		...
		if ( group_table[gn].rating[ch->archetype] <= 0 ) {
			act ( "$N tells you 'That group is beyond your powers.'",
				  ch, NULL, trainer, TO_CHAR );
			return;
		}

		if ( ch->train < group_table[gn].rating[ch->archetype] ) {
		...
		ch->train -= group_table[gn].rating[ch->archetype];
		return;
	}
	...
	if ( sn > -1 ) {
		...
		if ( skill_table[sn].rating[ch->archetype] <= 0 ) {
			act ( "$N tells you 'That skill is beyond your powers.'",
				  ch, NULL, trainer, TO_CHAR );
			return;
		}

		if ( ch->train < skill_table[sn].rating[ch->archetype] ) {
		...
		ch->train -= skill_table[sn].rating[ch->archetype];
		return;
	}
